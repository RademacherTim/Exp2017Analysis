...
, where $f_{m,17}$ is the fraction of the ring width formed in month $m$, $rw_{m, 15}$ and $rw_{m, 17}$ are respecitvely the 2015 and 2017 ring width in month $m$, and $rw_{N, 15}$ and $rw_{N, 17}$ are respectively the 2015 and 2017 ring widths as measured at the end of the growing season in November. This assumes that the ring was fully formed on the 1$^{st}$ of November, which is about two weeks after the typical end of cell wall thickening for white pine at Harvard Forest. Then, $f_{m, 17}$ theoretically varies between 0 and 1 with 0 meaning non of the ring was formed at the sampling date and 1 meaning that the entire ring was already formed.
...
Finally, the cell wall volume for each ring portion of the stem section ($V_{CW,s,p}$) is multiplied by a fixed cell wall density ($\rho_{CW}$) and carbon content () to estimate the total structural carbon gain per stem section and period ($M_{p,s}$ in g). Here, we use a cell wall density of `r rhoCW` $kgDM \, m^{−3}$ (measured for Scots pine by @plotze_porosity_2011) and carbon content of 49.74% for white pine [@lamlom_reassessment_2003].

$$
M_{s,p} = V_{CW,s,p} \times \rho_{CW} \times 49.74\%
$$
``` {r estimateDateOfFormation, echo = FALSE}
# TR - I should focus on the carbon in each section and not the estimated date of formation.
# If I want a date of formation I ought to  actually fit a Gompertz function to the percentage ring widths and interpolating using the Gompertz. At the moment the code only interpolates linearly between the sampling dates as hard boundaries, so the large majority of end dates falls on a sampling date. 
# 
# # add new column to data
...
# # iTree = 1
# # iHeight = 3
# # plot (x = as_date (c ('2017-07-5','2017-08-10','2017-10-09','2017-11-03')),
# #       y = c (f_Jul [iTree, iHeight], f_Aug [iTree, iHeight], f_Oct [iTree, iHeight], 1),
# #       typ = 'p', las = 1)
# 
...
#       }
#       
#       # estimate date of formation for each sector by linearly interpolating
#       if (numberOfZones > 0) {
...
# } # close period loop
# 
# # interpolate linearly for sample 06M which was fully formed by October sampling
# condition <- data [['YEAR']] == 2017 & data [['TREE']] == '06M'
# numberOfZones <- sum (condition, na.rm = TRUE)
# incr <- as.numeric (as_date ('2017-10-09') - as_date ('2017-03-01')) / (numberOfZones - 1)
# data [['formationDate']] [condition] <- seq (as_date ('2017-03-01'),
#                                              as_date ('2017-10-09'), by = incr)
# 
# 
# # interpolate linearly for sample 24A and 24B assuming it was fully formed by November
# condition <- data [['YEAR']] == 2017 & data [['TREE']] == '24A'
# numberOfZones <- sum (condition, na.rm = TRUE)
# incr <- as.numeric (as_date ('2017-11-03') - as_date ('2017-03-01')) / (numberOfZones - 1)
# data [['formationDate']] [condition] <- seq (as_date ('2017-03-01'),
#                                              as_date ('2017-11-03'), by = incr)
# condition <- data [['YEAR']] == 2017 & data [['TREE']] == '24B'
# numberOfZones <- sum (condition, na.rm = TRUE)
# incr <- as.numeric (as_date ('2017-11-03') - as_date ('2017-03-01')) / (numberOfZones - 1)
# data [['formationDate']] [condition] <- seq (as_date ('2017-03-01'),
#                                              as_date ('2017-11-03'), by = incr)
