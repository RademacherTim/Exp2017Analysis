...
for (i in 1:dim (data) [1]) {
  ...
  data [['cellTanWidth']] [i] <- data [['DRAD']] [i] + 2 * data [['CWTTAN']] [i] 
  data [['cellRadWidth']] [i] <- data [['DTAN']] [i] + 2 * data [['CWTRAD']] [i] 
  ...
  multiplier <- circumference / data [['cellTanWidth']] [i]
...
carbonContentData <- tibble (tree = rep (1:40, 4),
                             period = c (rep (as_date ('2017-07-03'), 40), 
                                         ...
                                         rep (as_date ('2017-10-09'), 40), 
                                         rep (as_date ('2017-11-03'), 40)),
                             structuralCGainat050 = NA, structuralCGainat100 = NA, 
                             structuralCGainat150 = NA, structuralCGainat200 = NA, 
                             structuralCGainat250 = NA)
...
### Treatment effects on anatomy

``` {r plotCWT, echo= FALSE, fig.width = 11}
# add new column to data
...
plotEndOfGrowingSeason <- function (y, treeLabels, colour, symbol) {
  ...
  # determine mean and standard deviation for the last day
  meanDate <- mean (lastDates [, 2], na.rm = TRUE)
  sdDate   <- sd   (lastDates [, 2], na.rm = TRUE)
  
  # and arrow bar for the standard deviation
  arrows (x0 = meanDate - sdDate, y0 = y, x1 = meanDate + sdDate, y1 = y, 
          code = 3, col = colour, 
          lwd = 2, angle = 90, lty = symbol - 18,
          length = 0.05)
  # plot point for mean date
  points (x = meanDate, y = y, pch = symbol, col = colour, bg = 'white', lwd = 2)
}

# plot CWT of the 2017 ring for each period and tree
layout (matrix (1:3, nrow = 1), widths = c (1.6, 1.6, 1))
par (mar = c (5,5,1,1))
...
plot (x = as_date (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] == '01M']),
      y = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] == '01M'],
      las = 1, col = 'white', typ = 'l',
      ylab = 'radial cell width (microns)',
      xlab = 'time',
      xlim = as_date (c ('2017-03-01','2017-11-03')),
      ylim = c (10, 50))
# plot all control trees
treeLabels <- c ('01M','03M','04M','06M','07M','09M','18M','30M','31M','36M')
controlMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                          by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                          mean)
lines (x = as_date (controlMean [, 1]),
       y = rollmean (controlMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# above the girld 
treeLabels <- c ('05A','11A','15A','16A','19A','23A','29A','35A','39A','40A')
aboveGirdlMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                             by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                             mean)
lines (x = as_date (aboveGirdlMean [, 1]),
       y = rollmean (aboveGirdlMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# below the girld 
treeLabels <- c ('05B','11B','15B','16B','19B','23B','29B','35B','39B','40B')
belowGirdlMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                             by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                             mean)
lines (x = as_date (belowGirdlMean [, 1]),
       y = rollmean (belowGirdlMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# above the compression
treeLabels <- c ('10A','12A','13A','17A','20A','21A','28A','32A','33A','38A')
aboveComprMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                             by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                             mean)
lines (x = as_date (aboveComprMean [, 1]),
       y = rollmean (aboveComprMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# below the compression
treeLabels <- c ('10B','12B','13B','17B','20B','21B','28B','32B','33B','38B')
belowComprMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                             by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                             mean)
lines (x = as_date (belowComprMean [, 1]),
       y = rollmean (belowComprMean [, 2], window, na.pad = TRUE, na.rm = T),
...
treeLabels <- c ('02A','08A','14A','22A','24A','25A','26A','27A','34A','37A')
aboveDCompMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                              by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                              mean)
lines (x = as_date (aboveDCompMean [, 1]),
       y = rollmean (aboveDCompMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# between the double compression
treeLabels <- c ('02M','08M','14M','22M','24M','25M','26M','27M','34M','37M')
betweenDCompMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                               by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                               mean)
lines (x = as_date (betweenDCompMean [, 1]),
       y = rollmean (betweenDCompMean [, 2], window, na.pad = TRUE, na.rm = T),
...
# below the double compression
treeLabels <- c ('02B','08B','14B','22B','24B','25B','26B','27B','34B','37B')
belowDCompMean <- aggregate (x  = data [['cellTanWidth']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels], 
                              by = list (data [['formationDate']] [data [['YEAR']] == 2017 & data [['TREE']] %in% treeLabels]), 
                              mean)
lines (x = as_date (belowDCompMean [, 1]),
       y = rollmean (belowDCompMean [, 2], window, na.pad = TRUE, na.rm = T),
       col = colours [4], lwd = 2, lty = 2)
legend (x = as_date ('2017-04-25'), lty = 1, lwd = 2,
        y = 25, box.lty = 0, bg = 'transparent', col = colours, legend = c ('control','girdled','compressed','double compression'))
legend (x = as_date ('2017-03-01'), lty = 2, lwd = 2,
        y = 22.7, box.lty = 0, bg = 'transparent', col = colours [2:4], legend = c ('','',''))
legend (x = as_date ('2017-03-30'), lty = 3, lwd = 2,
        y = 18.1, box.lty = 0, bg = 'transparent', col = colours [4], legend = '')
text (x = as_date (c ('2017-03-15','2017-04-15','2017-05-15')), 
      y = 25, labels = c ('below','middle', 'above'), cex = 0.9)
...
for (iTree in 1:40) { # loop over trees
  ...
  for (iHeight in heights) {
    ...
    nCells <- floor (sum (20 / data [['cellTanWidth']] [condition], na.rm = TRUE))
    cellNumber <- add_row (cellNumber, tree = iTree, treatment = treatment, height = iHeight, n = nCells)
...
cellNumber [cellNumber [['tree']] == 6, ] <- NA 
...
xPositions <- c (1.3, 1.5, 1.6, 1.8, 1.9, 2.1, 2.2, 2.3)
plot (x = xPositions, 
      y = cellNMeans [[3]] [c (1:6, 8, 7)], typ = 'p',
      col = c (colours [1], rep (colours [2], 2), rep (colours [3], 2), rep (colours [4], 3)),
      pch = c (19, 19, 21, 19, 21, 19, 21, 22),
      xlab = '', ylab = 'number of cells', las = 1, 
      xlim = c (1, 2.5), ylim = c (10, 80), xaxt = 'n', cex = 2.0)
axis (side = 1, at = xPositions, labels = c ('M','A','B','A','B','A','M','B'), tick = 0)
arrows (x0 = xPositions, 
        y0 = cellNMeans [[3]] [c (1:6, 8, 7)] - cellNSD [[3]] [c (1:6, 8, 7)] / sqrt (10), 
        y1 = cellNMeans [[3]] [c (1:6, 8, 7)] + cellNSD [[3]] [c (1:6, 8, 7)] / sqrt (10), 
        col = c (colours [1], rep (colours [2], 2), rep (colours [3], 2), rep (colours [4], 3)), 
        lwd = 2, lty = c (1, 1, 2, 1, 2, 1, 3, 2), code = 3, 
        angle = 90, length = 0.07)
points (x = xPositions, 
        y = cellNMeans [[3]] [c (1:6, 8, 7)],
        col = c (colours [1], rep (colours [2], 2), rep (colours [3], 2), rep (colours [4], 3)),
        pch = c (19, 19, 21, 19, 21, 19, 22, 21), lwd = 2, bg = 'white', cex = 2.0)
```
