...
```{r testDifferencesInWoodAnatomy, echo = FALSE}
# create long format tibble with ring width, carbon content, cell number, CWT and lumen diameter
woodAnatomy <-  tibble (tree = 1, treatment = 1, position = 'M', RW2017 = NA, CC = NA, n = NA, CWA = NA, CWT = NA, LD = NA)

for (iTree in 1:40) { # loop over trees
  
  # get treatment and determine heights
  treatment <- allometricData [['treatment']] [iTree]
  if (treatment == 1 & iTree != 6) {
    iPos <- 'M'
    iRW  <- as.numeric (standardisedRW2017 [iTree, 3])
    iCC  <- as.numeric (carbonContentData  [iTree, 5])
    iN   <- cellNumber [['n']] [cellNumber [['tree']] == iTree & cellNumber [['height']] == 'M' & !is.na (cellNumber [['n']])]
    iCWA <- max (data [['CWAACC']] [as.numeric (substr (data [['TREE']], 1, 2)) == treatment], na.rm = T)
    iCWT <- mean (data [['CWTALL']] [as.numeric (substr (data [['TREE']], 1, 2)) == treatment], na.rm = T)
    iLD  <- mean (data [['DRAD']] [as.numeric (substr (data [['TREE']], 1, 2)) == treatment], na.rm = T)
  } else if (treatment == 2 | treatment == 3) {
    iPos <- c ('A','B')
    iRW  <- as.numeric (standardisedRW2017 [iTree, c (2, 4)])
    iCC  <- as.numeric (carbonContentData  [iTree, c (6, 4)])
    iN   <- cellNumber [['n']] [cellNumber [['tree']] == iTree & cellNumber [['height']] %in% c ('A','B') & !is.na (cellNumber [['n']])]
    iCWA <- aggregate (data [['CWAACC']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree], 
                       by = list (data [['POS']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree]), max, na.rm = T) [[2]]
    iCWT <- aggregate (data [['CWTALL']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree], 
                       by = list (data [['POS']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree]), mean, na.rm = T) [[2]]
    iLD  <- aggregate (data [['DRAD']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree], 
                       by = list (data [['POS']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree]), mean, na.rm = T) [[2]]
  } else if (treatment == 4) {
    iPos <- c ('A','B','M')
    iRW  <- as.numeric (standardisedRW2017 [iTree, c (1, 5, 3)])
    iCC  <- as.numeric (carbonContentData  [iTree, c (7, 5, 3)])
    iN   <- cellNumber [['n']] [cellNumber [['tree']] == iTree & cellNumber [['height']] %in% c ('A','B','M') & !is.na (cellNumber [['n']])]
    iCWA <- aggregate (data [['CWAACC']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree], 
                       by = list (data [['POS']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree]), max, na.rm = T) [[2]]
    iCWT <- aggregate (data [['CWTALL']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree], 
                       by = list (data [['POS']] [as.numeric (substr (data [['TREE']], 1, 2)) == iTree]), mean, na.rm = T) [[2]]
  ...
  # add rows for tree
  woodAnatomy <- add_row (woodAnatomy, tree = iTree, treatment = treatment, position = iPos, 
                          RW2017 = iRW, CC = iCC, n = iN, CWA = iCWA, CWT = iCWT, LD = iLD)  
...
# delete first row
woodAnatomy <- woodAnatomy [-1, ]

# test for difference in RW2017
