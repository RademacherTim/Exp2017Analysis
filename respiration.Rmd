...
To estimate stem respiratory losses, weekly stem respiration measurements were made using a Li-Cor820 starting at 13.00h. The LiCor820 was attached to a PVC pipe (4" diameter) that were previously fitted and attached to the sections stems using silicone to create a closed system in which the enclosed air circulates via a small pump, similar to the Flux Puppy system [@citation]. Respiration rates were calculate from the change CO<sub>2</sub> using the RespChamberProc package [@citation] in R [@citation]. Stem sections were measured in the same order to minimise variations in time of day from week to week. The rates were corrected for water vapour pressure fluctuations using climate data from Fisher weather station for the 15 minute interval during which the measurement took place. 
...
$$
A_{s, i} = \frac {cbh_{s,i}} {100} * h 
$$
, where is the $cbh_{s,i}$ is the circumference in cemtimeters and $h$ is the height of the section in meters (here $h = 0.1$).
...
for (r in 1:dim (responseVariables) [1]) {
  ...
  if (predictors [['treatment']] [predictors [['tree']] == iTree] == 1) {
    responseVariables [['A150']] [r] <- 
      predictors [['cbh150']] [predictors [['tree']] == iTree] / 100.0 * h

  # Check whether this tree has two chambers (i.e. gridled or single compression)
  } else if (predictors [['treatment']] [predictors [['tree']] == iTree] == 2 | 
             predictors [['treatment']] [predictors [['tree']] == iTree] == 3) {
    responseVariables [['A100']] [r] <- 
      predictors [['cbh100']] [predictors [['tree']] == iTree] / 100.0 * h
    responseVariables [['A200']] [r] <- 
      predictors [['cbh200']] [predictors [['tree']] == iTree] / 100.0 * h

  # Check whether this tree has three chambers (i.e. double compression)
  } else if (predictors [['treatment']] [predictors [['tree']] == iTree] == 4) {
    responseVariables [['A50']]  [r] <- 
      predictors [['cbh50']]  [predictors [['tree']] == iTree] / 100.0 * h
    responseVariables [['A150']] [r] <- 
      predictors [['cbh150']] [predictors [['tree']] == iTree] / 100.0 * h
    responseVariables [['A250']] [r] <- 
      predictors [['cbh250']] [predictors [['tree']] == iTree] / 100.0 * h
  }
}
...
We then approximated the total loss of carbon for each combination of period and stem section by multiplying the geometric mean of spatially integrated respiration rates (g day-1) measured during each period by the length of the period ($l_p$ in days).

$$
R_{s,i} = \bar R_{s, i, j} \times l_p
$$
...
for (period in 1:4) {
  ...
  for (iTree in 1:41){
    ...
    if (predictors [['treatment']] [iTree] == 1) {
      responseVariables [['resp150']] [condition] <- tmpResp [['totalResp']] [tmpResp [['tree']] == iTree] 
    } else if (predictors [['treatment']] [iTree] == 2 | 
               predictors [['treatment']] [iTree] == 3) {
    ...
    } else if (predictors [['treatment']] [iTree] == 4) {
...
responseVariables <- add_column (responseVariables, treatment = rep (predictors [['treatment']], 4))
